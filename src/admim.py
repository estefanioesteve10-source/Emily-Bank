...
def painel_admin():
    print("\n" + "="*95) # adicionar qubra de linha e '=' 95 vezes
    ...
    try:
        # Seleciona exatamente as colunas que foram pedidas
        cursor.execute('''
            SELECT id, nome, iban, saldo, num_transferencias, num_depositos, num_levantamentos 
            FROM clientes
        ''')
        ...
        if not clientes:
            ...
            # Cabeçalho da tabela
            header = f"{'ID':<32}|{'NOME':<20} | {'IBAN':<28} | {'SALDO':<12} | {'TRANSF.':<8} | {'DEP/LEV':<10}"
            print(header)
            print("-" * 95)
            
            for c in clientes:
                id, nome, iban, saldo, transf, dep, lev = c
                # Formatação: nome (20 chars), iban (28 chars), saldo com 2 casas decimais
                info = f"{id[:32]:<32} |{nome[:20]:<20} | {iban:<28} | {saldo:>10.2f}€ | {transf:^8} | {dep}/{lev}"
                print(info)
